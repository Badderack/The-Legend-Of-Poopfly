...
class monster: #strukturen alla monster följer
    def __init__(self, genus, monstertyp, sty, kp):
        self.genus = genus
        self.monstertyp = monstertyp
        self.sty = sty
        self.kp = kp
    ...
    def generera_boss(sp1):
        bossmonsteralternativ = [ # möjliga bossar
            monster('Den store och mäktiga fritidsledaren: ', 'Mojje', random.randint(10, 30), sp1.niva+1 * random.randint(30,50)),
            monster('Den fruktansvärt (gulliga): ', 'Bleh', random.randint(5,15), sp1.niva+1 * random.randint(10,30)),
            monster('"Jag skulle behöva en önskan just nu..."', 'Mortecai', random.randint(10,20), sp1.niva+1 * random.randint(20,40)),
            monster('Den', 'den', 1, 1)
        ]
